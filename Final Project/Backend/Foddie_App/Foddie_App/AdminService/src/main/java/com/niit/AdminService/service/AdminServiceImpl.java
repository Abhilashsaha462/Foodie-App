...
public class AdminServiceImpl implements AdminService {
    ...
    public Restaurant addRestaurant(Restaurant restaurant) throws RestaurantAlreadyExistsException {
        if(adminRestaurantRepo.findById(restaurant.getEmail()).isPresent()){
            throw new RestaurantAlreadyExistsException();
        }

        return adminRestaurantRepo.save(restaurant);
    }

    @Override
    public Boolean removeRestaurant(String email) {
        adminRestaurantRepo.deleteById(email);
        return true;
    }
    ...
    public Admin updateRestaurant(String restaurantId,Restaurant restaurant){
        ...
        admin.getFavorite().removeIf(p -> p.getRestaurantId().equals(restaurant.getRestaurantName()));
